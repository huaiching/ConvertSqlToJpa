...
- 設定 專案的路徑
  ...
  - **檔案位置**: `file/project_path.txt`
  
  - 生成時，會使用這裡填寫的路徑，產生 `package` 和 `import` 等資訊。
    
    如：
    
    `com.mli` 就會在 entity 的檔案中 生成 
...
- 主鍵欄位
  ...
  - **內容格式**: 每行一個主鍵欄位名稱
    ...
    - 空白
      ...
      代表 table 無主鍵，會使用 table 全部欄位 來建立 複合組鍵
...
- 運行後，將會自動產生 spring data jpa 的相關檔案
  ...
  - **controller**：``file/output/controller``資料夾
    ...
    - 無主鍵：save, saveAll, update, deleteByEntity。
      無主鍵者，要自己處理 查詢方法。

### 3. 處理輸出檔案

- 將 `file/out` 資料夾中的 所有檔案 複製到 你的 Java 專案的對應資料夾中
  並 進行細部調整，例如：
  - `Entity` 的 未定義型態處理。
  - 各檔案之間的 import 設定。
...
- 如果未提供 `primary_keys.txt`，或 內容空白 者，視為 無主鍵 table 
